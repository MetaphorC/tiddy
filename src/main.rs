...
impl Layout {
    fn available_pos(&self) -> Vec2 {
        use LayoutKind::*;
        match self.kind {
            Vert => self.pos + self.size * Vec2::new(1, 0),
            Horz => self.pos + self.size * Vec2::new(0, 1),
        }
    }
...
fn parse_item(line: &str) -> Option<(Status, &str)> {
    let todo_item =
        line.strip_prefix("TODO: ").map(|title| (Status::Todo, title));
    let done_item =
        line.strip_prefix("DONE: ").map(|title| (Status::Done, title));

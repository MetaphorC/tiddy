use ncurses::*;
use std::fs::File;
use std::io::{self, Write, ErrorKind, BufRead};
use std::env;
use std::process;
use std::ops::{Add, Mul};
use std::cmp;
                    ...
                    if panel == Status::Todo {
                        ...
                        if let Some(key) = ui.key.take() {
                            match key as u8 as char { 
                                ...
                                'G' => list_last(&mut todos, &mut todo_curr),
                                '\n' => {
                    ...
                    if panel == Status::Done {
                        ...
                        if let Some(key) = ui.key.take() {
                            match key as u8 as char { 
                                ...
                                'i' => {
                                    notification.push_str(
                                        "Can't insert new DONE items, only DONE allowed."
                                    );
                                }
                                'd' => {
                                    list_delete(&mut dones, &mut done_curr);
                                    notification.push_str("BURN TO ASHES!");
                                }
                                'k' => list_up(&mut done_curr),
                                'j' => list_down(&dones, &mut done_curr),
                                'g' => list_first(&mut done_curr),
                                'G' => list_last(&mut dones, &mut done_curr),
                                '\n' => {
                        ...
                        for done in dones.iter() {
                            ui.label_fixed_width(&format!("- [ ] {}", done), x / 2, REGULAR_PAIR);
                        }
                    }
                }
                ui.end_layout();
